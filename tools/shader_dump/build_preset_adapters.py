...
Keeps the modern shader programs, paired stages, coverage and depth contracts.
The recipe pins source programs, exports, preset files and a complete modern
...
from build_shader_adapter import assembly_at, assemble, instructions
import d3d9bc
from enb163_hash import filename
from make_shader_aliases import leaf, sha

ROOT = Path(__file__).resolve().parents[2]
RECIPES = ROOT / "research/contracts/modern-preset-adapters.json"
...
def verify_variant(corpus, expected):
    ...
    if set(paths) != set(expected):
        raise ValueError("Modern corpus is incomplete or has extra containers")
    for name, digest in expected.items():
        if sha(paths[name].read_bytes()) != digest:
            raise ValueError(f"Modern corpus changed: {name}")
    return paths
        ...
        for slot, shader in enumerate(d3d9bc.extract_file(path)):
            ...
            if name in outputs:
                if sha(shader.data) != outputs[name]["original_sha256"]:
                    raise ValueError(f"Modern alias collision: {name}")
                affected[name].append({"container": path.name, "slot": slot})
    if any(not slots for slots in affected.values()):
        raise ValueError("Generated alias has no target in the modern corpus")
    return affected
...
def build(legacy, modern, preset, corpus, assembler, output, contract):
    legacy, modern, preset, corpus, assembler, output = [Path(p).resolve() for p in
        (legacy, modern, preset, corpus, assembler, output)]
    if output.exists() or any(output.is_relative_to(p) or p.is_relative_to(output)
                              for p in (legacy, modern, preset, corpus)):
    ...
    paths = verify_variant(corpus, contract["modern_corpus"])
    check_exports(legacy, contract["legacy_exports"])
    check_exports(modern, contract["modern_exports"])
    outputs, reports = {}, []
    with tempfile.TemporaryDirectory(prefix="enb-preset-") as scratch:
        ...
        for entry in entries:
            ...
            for source in entry["sources"]:
                ...
                mapping = audit(read_export(legacy / (container + ".xml")),
                                read_export(modern / (container + ".xml")), [slot])["entries"][0]
            ...
            if entry["status"] == "no_preset_delta":
                ...
                if not entry["targets"]:
                    raise ValueError("Adapter has no modern target")
                ...
                if supplied != mapped:
                    raise ValueError("Recipe does not cover all mapped modern programs")
                for target in entry["targets"]:
                    container, slot = leaf(target["container"]), target["slot"]
                    if sha((modern / container).read_bytes()) != contract["modern_corpus"][container]:
                        raise ValueError("Modern export and full corpus differ")
                    original = d3d9bc.extract_file(modern / container)[slot]
                    if sha(original.data) != target["sha256"]:
                        raise ValueError("Modern target program changed")
                    export = read_export(modern / (container + ".xml"))
                    roles = [role for role, p in export["passes"].items() if p[original.stage] == slot]
                    if roles != target["roles"]:
                        raise ValueError("Modern target roles changed")
                    ...
                    text = assembly_at(modern / (container + ".xml"), slot)
                    if target.get("validation_modern_fixture"):
                        verify_fixture(target["validation_modern_fixture"], text)
                    stem = f"{name}-{container}-{slot}"
                    free_registers(text, **target.get("scratch", {"constants": [], "temporaries": [], "samplers": []}))
                    compiled = assemble(assembler, text.replace("\0", ""), work, stem + "-modern")
                    if compiled.stripped() != original.stripped():
                        raise ValueError("Modern export does not reproduce the original program")
                    ...
                    row["targets"].append({"container": container, "slot": slot, "roles": roles,
                        "stage": original.stage, "alias": alias, "modern_sha256": sha(original.data),
        ...
        report = {"format": 1, "backend": "exact_1040_reviewed_delta", "game_modified": False,
                  "rendering_validated": False, "requires": contract["requires"],
                  "modern_corpus": contract["modern_corpus"],
...
def main():
    ...
    for option in ("legacy-exports", "modern-exports", "preset", "modern-corpus", "assembler", "out"):
        ap.add_argument("--" + option, type=Path, required=True)
    ap.add_argument("--recipes", type=Path, default=RECIPES)
    args = ap.parse_args()
    report = build(args.legacy_exports, args.modern_exports, args.preset, args.modern_corpus,
