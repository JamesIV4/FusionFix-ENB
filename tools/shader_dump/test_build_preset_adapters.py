...
from build_preset_adapters import (RECIPES, apply_edits, collision_scan,
                                   free_registers, verify_fixture, verify_variant)
from build_shader_adapter import instructions
from make_shader_aliases import sha
...
class PresetAdapterTests(unittest.TestCase):
    ...
    def test_arithmetic_fixture_must_match_the_actual_shader_and_stay_in_the_fixture_tree(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('build_preset_adapters.ROOT', directory):
    ...
    def test_collisions_are_checked_against_actual_program_bytes(self):
        ...
        with patch('build_preset_adapters.filename', return_value='psh11111111.txt'), \
             patch('build_preset_adapters.d3d9bc.extract_file', return_value=[Shader(b'changed')]):
    ...
    def test_checked_in_contract_accounts_for_all_twelve_inputs(self):
        ...
        self.assertEqual(len(contract['modern_corpus']), 103)
    ...
    def test_normal_translation_preserves_live_modern_coverage_register(self):
        contract = json.loads(RECIPES.read_text(encoding='utf-8'))
        ...
        # r1.x carries modern coverage through all new normal/specular work.
    ...
    def test_billboard_changes_only_wd_draw_and_matches_legacy_wind_ratio(self):
        ...
        # Modern VS supplies 16*color+8; the preset uses 32*color+16. Its
        # offset changes from 1/640 to .001, preserving modern size scaling.
    ...
    def test_specular_register_relocation_keeps_shininess_and_coverage(self):
        # Old r2.w and modern r2.x carry shininess; old r2.x and modern r2.w
            ...
            for detail in (-.7, .25, 1.):
                for shininess in (.03, 4., 80.):
                    for name, code in recipes.items():
                        ...
                        modern = {'r12.w': shininess, 'r12.x': spec, 'r11.w': detail,
                                  'c177.z': 1., 'r1.x': .37}
                        execute(code, modern)
                        self.assertAlmostEqual(modern['r2.w'], expected)
                        self.assertEqual(modern['r2.x'], shininess)
                        self.assertEqual(modern['r1.x'], .37)
